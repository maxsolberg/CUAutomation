...
class GraphGUI:
    ...
    def __init__(self, graph, width=800, height=800):
        ...
        win.setCoords(-1 * self.graph.COORD_BOUNDS - 20, -1 * self.graph.COORD_BOUNDS -
                      20, self.graph.COORD_BOUNDS + 20, self.graph.COORD_BOUNDS + 20)

        info = Text(Point(0, self.graph.COORD_BOUNDS + 10),
                    "Seed: " + str(self.graph.seed))
        info.draw(win)
